...
document.addEventListener('DOMContentLoaded', async () => {
    // 1. Initial Load Prices
    await loadPrices();

    // 2. Initialize 3D (function from app3d.js)
...
async function loadPrices() {
    try {
        ...
        data.materials.forEach(m => {
            ...
            opt.value = m.price;
            opt.textContent = `${m.name} - ${m.price.toLocaleString('ru-RU')} ₸/м²`;
...
function setupEventListeners() {
    ...
    ['input', 'change'].forEach(eventType => {
        [roofShapeInput, materialSel, wasteInput, sheetWidthInput, laborInput, overhangInput, includeInsulationCb, includeWoodCb].forEach(el => {
            if (!el) return;
            el.addEventListener(eventType, () => {
                window.needsCalcUpdate = true;
                if (el === materialSel || el === roofShapeInput || el === overhangInput) {
                    window.needs3DUpdate = true;
                }
                if (el === roofShapeInput) {
                    ...
                    if (roofShapeInput.value === 'flat') {
                        ...
                        angleLabelText.textContent = roofShapeInput.value === 'mansard' ? 'Крутизна нижнего ската:' : 'Угол наклона скатов:';
                    }
    ...
    if (resetBtn) {
        resetBtn.addEventListener('click', async () => {
            ...
            await loadPrices(); // Reset prices and defaults
            renderDormerUI();
            window.needs3DUpdate = true;
            recalculateNumbers();
    ...
    window.dormerConfig.forEach((dormer, index) => {
        ...
        el.innerHTML = `
            ...
            <div class="form-group" style="margin-bottom:0;">
                ...
                <input type="range" min="-${maxPos}" max="${maxPos}" step="0.1" value="${dormer.position}" oninput="updateDormer(${index}, 'position', this.value)">
            </div>
        `;
...
window.updateDormer = (index, key, val) => {
    ...
    if (key !== 'position') renderDormerUI();
        ...
        const label = document.querySelector(`#dormersContainer .dormer-item:nth-child(${index + 1}) label span`);
        if (label) label.textContent = val + ' м';
    }
    window.needs3DUpdate = true;
    recalculateNumbers();
};
...
async function _recalculateNumbers() {
    ...
    try {
        ...
        if (data.detailedMaterials) {
            const block = document.getElementById('detailedMaterialsBlock');
